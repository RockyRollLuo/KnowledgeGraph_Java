...
f = open("../../FB15k/e1_e2.txt","r")
for line in f:
    seg = line.strip().split()
    ok[seg[0]+" "+seg[1]] = {}
    ok[seg[1]+" "+seg[0]] = {}
f.close()
print "read e1_e2 over"

###g = open("data/path2.txt","w")
g = open("../../PCRA2/path2.txt","w")
...
g.close()
def work(dir):
    f = open("../../FB15k"+dir+".txt","r")
    ##g = open("data/"+dir+"_pra.txt","w")
    g = open("../../PCRA2/"+dir+"_pra.txt","w")
    for line in f:
        seg = line.strip().split()
        e1 = seg[0]
        e2 = seg[1]
